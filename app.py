...
# -----------------------------
# 한글 폰트 설정
# plt.rcParams 변경 대신 FontProperties 객체를 직접 전달하는 방식으로
# 환경에 관계없이 한글이 안정적으로 렌더링됩니다.
# -----------------------------
...
st.sidebar.markdown("---")
st.sidebar.info("본 앱은 서울시 상수도 노후화 및 수질 영향 분석 보고서를 바탕으로 만든 러프 버전입니다.")
...
# -----------------------------
    ...
    ax.set_xticklabels(labels, rotation=45, ha="right", fontproperties=kf(10))
    ax.set_yticklabels(labels, fontproperties=kf(10))
    ax.set_title("상관관계 히트맵", fontproperties=kf(13))
...
# -----------------------------
    ...
    ax.set_xlabel("사용연수", fontproperties=kf(11))
    ax.set_ylabel("탁도", fontproperties=kf(11))
    ax.set_title("사용연수 vs 탁도", fontproperties=kf(13))
    ax.legend(prop=kf(10))
